export class EcAttribution {
  ...
  log = () => {
    ...
    if (typeof window.heap != "undefined") {
      if (window.heap.identity) { console.log('[$EC] Heap ID: ', window.heap.identity); }
      else { console.log('[$EC] Heap ID: Not Set'); }
    } else {
  ...
  mkto = {
    blindForm: {
      load: (config: MktoBlindFormConfig) => {
        ...
        mktoForms2Script.addEventListener("load", () => {
          ...
          window.MktoForms2.whenRendered( (mktoForm: any) => { // eslint-disable-line  @typescript-eslint/no-explicit-any
            ...
            if (config.followupUrl) {
              ...
              if (this.debug) {console.log(`[$EC] Followup URL Set: ${config.followupUrl}`);}
            }

            if (config.bindFields) {
              // Bind main form field to mkto form fields
              if (typeof config.dataMap != undefined) {
                if (this.debug) {console.log("[$EC] Binding fields...", config.dataMap);}
              ...
              if (typeof config.dataMap != undefined) {
                if (this.debug) {console.log("[$EC] Populating with provided data...", config.dataMap);}
          ...
          if (config.submitOnLoad && config.submitData) {
            if (this.debug) {console.log("[$EC] Submitting Marketo form...");}
            window.MktoForms2.whenReady( (mktoForm: any) => { // eslint-disable-line  @typescript-eslint/no-explicit-any
              mktoForm.submit();
            })     
          }
        });
    ...
    kioskMode: () => {
      ...
      // Forms are rendered when initially created, then again every 
      // time that visibility rules alter the structure of the form.
      window.MktoForms2.whenRendered( (form: any) => {  // eslint-disable-line  @typescript-eslint/no-explicit-any
  ...
  protected handleMktoForms() {
    ...
    window.MktoForms2.whenReady( (form: any) => {  // eslint-disable-line  @typescript-eslint/no-explicit-any
